...
public class BoardRepositoryTests {
    ...
    public void testSelect(){
        ...
        // 널값이 나올 경우를 대비한 객체
        //                                      .fildById(bno);  -> select * from board where bno = bno;
    ...
    public void testUpdate(){
        ...
        Board board = result.orElseThrow(); // 가져온 값이 있으면 board타입에 객체에 넣는다.

        board.change("수정 테스트 제목", "수정 테스트 내용"); // 제목과 내용만 수정할 수 있는 메서드

        boardRepository.save(board); // save 메서드는 pk값이 없으면 insert, pk값이 있으면 update 함.
    ...
    public void testPaging(){
        ...
        // Page 클래스는 다음페이지 존재여부, 이번페이지 존재여부, 전체 데이터 개수, 등등... 계산을 한다.
        ...
        //        forEach는 인덱스를 사용하지 않고 앞에서 부터 객체를 리턴함
